...
export const DashboardLayout = () => {
  ...
  return (
    <div 
      className="min-h-screen w-full p-4 md:p-8 flex flex-col overflow-hidden transition-all duration-500"
      style={{
    ...
    >
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="flex flex-col xl:flex-row items-stretch xl:items-end justify-between gap-6 w-full mb-8 relative z-20">
        ...
        <div 
          className="flex-1 min-w-[300px] border-b-2 pb-2 flex gap-4 items-center relative"
          style={{ borderColor: matrixColor.hex }}
        >
          {/* Avatar Container */}
          <div 
            className="w-12 h-12 border-2 flex-shrink-0 flex items-center justify-center font-primary text-2xl font-bold bg-[#0a0a1a] overflow-hidden"
            style={{ 
              borderColor: matrixColor.hex, 
              color: matrixColor.hex,
              boxShadow: `0 0 15px ${matrixColor.hex}66`
            }}
          >
          ...
          <div className="flex-1 flex flex-col justify-center gap-1.5 mt-1">
            <div className="flex justify-between items-end">
              <span className="font-primary text-sm md:text-base tracking-[0.15em] uppercase truncate max-w-[200px]" style={{ color: matrixColor.hex }}>
            ...
            {/* Experience Bar */}
            <div className="w-full h-2.5 bg-primary border relative overflow-hidden" style={{ borderColor: `${matrixColor.hex}80` }}>
              <div 
                className="absolute top-0 left-0 h-full transition-all duration-1000 ease-out" 
                style={{ 
                  width: `${expProgress}%`,
                  backgroundColor: matrixColor.hex,
                  boxShadow: `0 0 10px ${matrixColor.hex}`
                }} 
              />
            </div>
          </div>
        ...
        <div className="flex-[1.5] w-full min-w-[300px] relative h-10 bg-secondary flex items-center overflow-hidden border border-light" style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 12px 100%, 0 calc(100% - 12px))' }}>
          <div className="absolute bottom-0 left-0 w-4 h-4 bg-accent" style={{ clipPath: 'polygon(0 100%, 100% 100%, 0 0)' }}></div>
        ...
        <div className="flex-1 min-w-[250px] border-b-2 pb-2 flex flex-col items-end justify-center relative" style={{ borderColor: `${matrixColor.hex}CC` }}>
          <div className="flex items-center gap-2 mb-1.5">
            <div className="w-2 h-2 rounded-full animate-pulse" style={{ backgroundColor: matrixColor.hex, boxShadow: `0 0 8px ${matrixColor.hex}` }}></div>
            <span className="font-primary text-[10px] tracking-widest" style={{ color: matrixColor.hex }}>SYNC ACTIVE</span>
          </div>
          <div className="font-primary text-xs md:text-sm tracking-[0.2em]" style={{ color: `${matrixColor.hex}CC` }}>STARDATE: {stardate}</div>
        </div>
      </motion.div>
