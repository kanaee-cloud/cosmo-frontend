...
export const CommsRelayModal = ({ isOpen, onClose }) => {
  ...
  const { useInbox, searchCaptains, sendFriendRequest, acceptRequest } = useSocial();
  const { data: notifications, isLoading: inboxLoading } = useInbox();

  // Handle Search dengan sedikit delay (Debounce manual sederhana)
  useEffect(() => {
              ...
              {inboxLoading ? (
                ...
                notifications?.map(notif => (
                  <div key={notif.id} className={`p-4 border ${notif.is_read ? 'border-gray-800 bg-[#0a0a1a]/30' : 'border-[#3d2278]/80 bg-[#3d2278]/10'} flex items-center justify-between gap-4`}>
                    ...
                    {/* Tombol Aksi (Khusus Friend Request yang belum dibaca) */}
                    {notif.type === 'FRIEND_REQUEST' && !notif.is_read && (
                      <button 
                        onClick={() => acceptRequest.mutate({ friendshipId: notif.related_id, notificationId: notif.id })}
