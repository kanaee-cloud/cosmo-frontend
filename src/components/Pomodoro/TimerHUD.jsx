    ...
    <div className="relative flex flex-col items-center mb-16 mt-8 w-full">
      ...
      <button 
        ...
        className={`absolute -top-8 right-10 md:right-1/4 z-30 px-3 py-1 border font-secondary tracking-widest text-[10px] uppercase transition-all duration-300 ${
          isLocked 
            ? 'border-tertiary/30 bg-primary/20 text-tertiary cursor-not-allowed' 
      ...
      >
        ...
        <h1 className={`font-press text-5xl md:text-6xl z-10 transition-colors duration-500 ${currentPhase === 'break' ? 'text-light' : 'text-accent'}`} style={{ textShadow: `0 0 ${isRunning ? '30px' : '20px'} ${currentPhase === 'break' ? 'rgb(var(--color-light) / 0.6)' : 'rgb(var(--color-accent) / 0.8)'}` }}>
          {formatTime(timeLeft)}
        </h1>
      </div>
      ...
      <div className={`absolute -bottom-4 border px-4 py-1.5 z-20 transition-colors duration-500 shadow-[0_4px_20px_rgb(var(--color-primary))] ${
        ...
        <span className={`font-secondary text-[9px] md:text-[10px] tracking-[0.2em] transition-colors duration-500 uppercase ${
          isRunning 
            ? (currentPhase === 'break' 
                ? 'text-light' 
