  ...
  const handleNext = () => {
    ...
    setTimeout(() => {
      if (currentQuestion < quizQuestions.length - 1) {
        ...
        // Final score calculation could happen here
        const finalScore = isCorrect ? score + 1 : score;
        onQuizComplete(finalScore, quizQuestions.length);
      }
    }, 1500); // Delay to show result feedback
  };
          ...
          {quizFinished ? (
            <div className="text-center py-10">
              ...
              <p className="font-secondary text-cyan-400 text-lg mb-8">
                SCORE: {score + (selectedOption === quizQuestions[currentQuestion].correct_index ? 1 : 0)} / {quizQuestions.length}
              </p>
              <button onClick={onClose} className="px-8 py-3 bg-cyan-500 text-[#0a0a1a] font-primary font-bold tracking-widest hover:bg-cyan-400 transition-colors">
                RETURN TO DECK
              </button>
            </div>
          ) : (
