  ...
  return (
    <div className="flex justify-center items-end gap-12 md:gap-20 h-[26rem] font-secondary mb-12 w-full pt-8">
      {podium.map((pilot, index) => {
        ...
        // Semantic Colors menggantikan warna statis
        ...
        return (
          <div key={pilot.rank} className="flex flex-col items-center">
            ...
            <div className="h-6 mb-2">
              {pilot.title && (
                <div className="text-accent text-xs tracking-widest text-center font-press drop-shadow-[0_0_5px_rgb(var(--color-accent)_/_0.5)] transition-colors duration-500">
                  {pilot.title}
                </div>
              )}
            </div>
            ...
            {/* Hollow Box Avatar */}
            <div className={`w-10 h-12 border-2 mb-4 transition-colors duration-500 ${borderColor}`}></div>
            
            {/* Username */}
            <div className="text-text text-xs mb-3 tracking-widest uppercase font-press transition-colors duration-500">
              {pilot.username}
            </div>
